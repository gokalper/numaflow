...
impl ShmRingBuffer {
    pub fn new<P: AsRef<Path>>(path: P, size: usize) -> Result<Self> {
        ...
        file.set_len((size + std::mem::size_of::<RingBufferHeader>()) as u64)
            .map_err(|e| {
                Error::SharedMemory(format!("Failed to set shm file length: {}", e).into())
            })?;

        let mut mmap = unsafe {
            MmapMut::map_mut(&file).map_err(|e| {
                Error::SharedMemory(format!("Failed to mmap shm file: {}", e).into())
            })?
        };
    ...
    pub fn write_message(
        ...
        let mut header =
            ShmPacketHeader::new(payload_len as u32, epoch_id, generation_id, partition_id);
        ...
        // 4. Validate Header (Flags = 1)
        {
            ...
            if first_chunk < header_size {
                buffer[0..(header_size - first_chunk)]
                    .copy_from_slice(&valid_header_bytes[first_chunk..header_size]);
            }
        }
        ...
        header_ring
            .head
            .store((head + total_len) % self.capacity, Ordering::Release);

        Ok(total_len)
    }

    /// Write data to shared memory (deprecated, use write_message)
    /// Phase 1 TODO: Will be used for UDF data path
    #[allow(dead_code)]
    ...
    pub fn read_exact(&mut self, len: usize) -> Result<Vec<u8>> {
        ...
        unsafe {
            ...
            if first_chunk < len {
                ...
                std::ptr::copy_nonoverlapping(
                    chunk2_src,
                    result.as_mut_ptr().add(first_chunk),
                    remaining,
                );
            }
        }
        ...
        self.header()
            .tail
            .store((tail + len) % self.capacity, Ordering::Release);

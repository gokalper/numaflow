...
pub struct ShmMapClient {
    ...
    generation_id: u64,
    tracker: Option<Tracker>, // For fence checks
}

impl ShmMapClient {
    pub fn new(
        request_buffer: ShmRingBuffer,
        response_buffer: ShmRingBuffer,
        generation_id: u64,
    ...
    async fn map(
        ...
        tokio::spawn(async move {
            while let Some(req) = input_stream.next().await {
                // Phase 1.4: Fence check to prevent zombie writes
                if let Some(ref t) = tracker {
                    if t.is_fenced() {
                        tracing::info!("SHM writer fenced, stopping");
                        break;
                    }
                }
                ...
                if req.encode(&mut buf).is_ok() {
                    // Adaptive backoff: 10μs → 100μs → 1ms
                    let mut backoff_us: f64 = 10.0;
                    loop {
                        ...
                        match ring.write_message(&buf, 0, generation_id, 0) {
                            Ok(0) => {
                                // Full, adaptive backoff to reduce tail latency
                                drop(ring);
                                tokio::time::sleep(Duration::from_micros(backoff_us as u64)).await;
                                backoff_us = (backoff_us * 1.5).min(1000.0); // Cap at 1ms
                            }
                            Ok(_) => break, // Success
                            Err(e) => {
        ...
        let tracker_read = self.tracker.clone();
        let (tx, rx) = mpsc::channel(128);

        // Reader Task
        tokio::spawn(async move {
            let mut backoff_us: f64 = 10.0; // Adaptive backoff
            loop {
                // Fence check
                if let Some(ref t) = tracker_read {
                    if t.is_fenced() {
                        tracing::info!("SHM reader fenced, stopping");
                        break;
                    }
                }
...

                {
                    ...
                    if let Ok(bytes) = ring.peek_exact(header_size) {
                        if let Some(header) =
                            crate::shared::shm::ShmPacketHeader::from_le_bytes(&bytes)
                        {
                            ...
                            if !magic_valid || !version_valid || !flags_valid {
                                ...
                                tracing::error!(
                                    "Invalid SHM Header (magic/ver/flags: {}/{}/{}). Terminating.",
                                    header.magic,
                                    header.version,
                                    header.flags
                                );
                                ...
                                tracing::error!(
                                    "Generation ID Mismatch! Expected {}, Got {}. Revoked? Terminating.",
                                    generation_id,
                    ...
                    if let Some(data) = payload {
                        match MapResponse::decode(&data[..]) {
                            ...
                            Err(e) => {
                                ...
                                if tx
                                    .send(Err(Status::internal("Decode Error")))
                                    .await
                                    .is_err()
                                {
                                    break;
                        ...
                        backoff_us = 10.0; // Reset on success
                        continue; // Data was read, try reading next immediately
                    }
                }

                // If no data or partial data, adaptive backoff
                tokio::time::sleep(Duration::from_micros(backoff_us as u64)).await;
                backoff_us = (backoff_us * 1.5).min(1000.0); // 10μs → 1ms
            }
        });
